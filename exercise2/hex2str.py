...
if sys.argv[1] == "-d":
    ...
    if not isBase64(data):
        try:
            ...
            for c in data:
                print(''.join(random.choice(string.ascii_lowercase) for i in range(20)), file=sys.stderr, end="", flush=False)
                print(c, end="", flush=True)
                print(''.join(random.choice(string.ascii_lowercase) for i in range(20)) + " ", file=sys.stderr, flush=True)
            print()
